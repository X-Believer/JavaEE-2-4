...
public class ProductVo {
    ...
    public ProductVo(Product product, Integer type) {
        super();
        CloneFactory.copy(this, product);
        if(type == 1 || type == 2) CloneFactory.copy(this, product.getValidOnsale());
        if(type == 2) this.setfreightTemplate(IdNameTypeVo.builder().id(product.getTemplate().getId()).name(product.getTemplate().getName()).build());
    }
