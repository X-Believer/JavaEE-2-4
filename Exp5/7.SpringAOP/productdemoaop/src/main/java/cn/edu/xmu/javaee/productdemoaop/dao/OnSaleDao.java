...
public class OnSaleDao {
    ...
    @Autowired
    public OnSaleDao(OnSalePoMapper onSalePoMapper, SaleRepository saleRepository) {
    ...
    public List<OnSalePo> getLatestOnSale_JPA(Long productId) throws DataAccessException {
        LocalDateTime now = LocalDateTime.now();
        OnSalePoExample example = new OnSalePoExample();
        return saleRepository.getOnsaleList(productId);
    }
}
